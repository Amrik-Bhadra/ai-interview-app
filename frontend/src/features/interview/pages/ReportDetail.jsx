...
// TODO: replace with a real fetch using `id` from your reports API
const sampleReport = {
  ...
  preparationPlan: [
    { day: 1, focus: "Advanced Database Query Tuning", tasks: ["Review MongoDB execution plans via .explain() logs.", "Solve 3 optimization challenges regarding compound index strategies."] },
    { day: 2, focus: "Caching Strategies & Distributed Operations", tasks: ["Implement a standalone Cache-Aside pattern middleware using a local Node-Redis client library setup.", "Outline 3 specific edge-case solutions regarding cache stampedes or invalidation loops."] },
  ],
};
    ...
    <div className="report-detail">
      <div className="report-header">
        <div className="header-text">
          ...
          <p className="header-sub">Generated against the job description and resume below.</p>
        </div>
        <ScoreGauge score={report.matchScore} />
      </div>

      <div className="summary-grid">
        <div className="summary-card"><p className="block-label">Job description</p><p>{report.jobDescription}</p></div>
        <div className="summary-card"><p className="block-label">Resume summary</p><p>{report.resumeText}</p></div>
        <div className="summary-card"><p className="block-label">Self description</p><p>{report.selfDescription}</p></div>
      </div>
      ...
      <div className="skillgap-section">
        ...
        <div className="skillgap-list">
          {report.skillGaps.map((gap) => (
            <span key={gap.skill} className={`skillgap-badge ${gap.severity}`}>
              {gap.skill}
              <em>{gap.severity}</em>
            </span>
          ))}
      ...
      <div className="questions-section">
        ...
        <div className="question-list">
          {report.technicalQuestions.map((q, i) => <QuestionAccordion key={i} index={i} {...q} />)}
        </div>
      </div>
      ...
      <div className="questions-section">
        ...
        <div className="question-list">
          {report.behavioralQuestions.map((q, i) => <QuestionAccordion key={i} index={i} {...q} />)}
        </div>
      </div>
          ...
          {report.preparationPlan.map((day) => (
            <div className="prep-day" key={day.day}>
              ...
              <div className="prep-day-content">
                <h4>{day.focus}</h4>
                <ul>{day.tasks.map((task, i) => <li key={i}>{task}</li>)}</ul>
              </div>
            </div>
          ))}
        </div>
      ...
      <Link to="/reports" className="back-link">&larr; Back to all reports</Link>
    </div>
  );
};
